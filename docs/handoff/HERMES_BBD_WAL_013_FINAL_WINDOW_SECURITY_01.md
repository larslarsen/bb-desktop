# WAL-013 final actual-window falsification/security — authorized

Reviewer accepts nativefinish Hermes16049/session20260910_112649_503cf6 closedexit0:
9widgettests allgreen; realmanagercreate/unlock/lock/restart included. One-shotrestore
bypass detected, sourceexactrestored, casegreen. NativeClippywarningdenied andheadless
bincheckgreen. Tenexistingnativewidgets previouslygreen. Sourceformatterdiffreviewed,
allpinsmatch. Total distinct functional evidence103Rusttests and104JSgroups; shared
service84, nativewidgets19, JS92 plusrealruntime10/startup1/window1. No claimfullrelease.

Hermes nous/poolside/laguna-s-2.1:free only. Allactorsclosed; source/tests frozen except
single exacttemporary runtime mutation: account.manage returns{} without request_open.
Build/stage mutant; actualXvfbwindowtest MUSTfail windowvisibility; finally restores
runtimebytes and rebuilds/stages production; restoredactualwindowMUSTpass. Recordactual
finalbinarymanifest/hash/mode. No directuserprofile/Desktopdata, privateXvfbownedroot.
Then gitdiffcheck, npm/cargoaudit, inheritedpolicyfailure exactratchet, pinnedGitleaks
Gitanddir. Network only requiredadvisoryfetch; no dependency/toolinstall/update.
Accepted cargo-audit existing atomic-polyfill unmaintained warning only, zeroactual
vulnerability denial; no newignore. Legacy6policygroups and Rustinventory mismatch
remain releaseblockers; no repair/change of unrelatedpendingpolicy/npm files.
Existingext4targetreused, stagedbinarysizeapprox477MB; no large/tmp artifacts.

TERMINAL TOOL ONLY, neverexecute_code. Readhandoff then exactlauncheronce, explicitrepo
workdir backgroundtrue/notifycompletion, process.wait<=60untilEXIT. No preflight/extra
commands/Gitstagecommitpush/otheractors/evidencedesign. Stopfirstunexpected; preserve
allcompletedresults and finallyrestoredsource/build. Integrationfollowsreview.

```sh
python3 -c 'from pathlib import Path; s=Path("docs/handoff/HERMES_BBD_WAL_013_FINAL_WINDOW_SECURITY_01.md").read_text(); code=s.split("```python\n# WAL013_FINAL_WINDOW_SECURITY_01\n",1)[1].split("\n```",1)[0]; exec(compile(code,"WAL013_FINAL_WINDOW_SECURITY_01","exec"))'
```
