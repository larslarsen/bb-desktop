...
const root = 'wallet-broker/target/wal017-diagnostic-sameconnection';
let sharedClient;
...
function rpc(method,body,label) {
  return new Promise(resolve => {
    const started=Date.now();const client=sharedClient;
...
(async()=>{
  ...
  sharedClient=http2.connect(endpoint);
  await new Promise((resolve,reject)=>{const timer=setTimeout(()=>reject(new Error('connect deadline')),15000);sharedClient.once('connect',()=>{clearTimeout(timer);resolve();});sharedClient.once('error',()=>{clearTimeout(timer);reject(new Error('connection'));});});
  ...
  fs.writeFileSync('docs/testing/BBD-WAL-017-DIAGNOSTIC-02.md','# WAL017 bounded public RPC diagnostic\n\n```json\n'+JSON.stringify(record,null,2)+'\n```\n',{flag:'wx'});
})().catch(()=>{if(sharedClient)sharedClient.destroy();console.error('Diagnostic stopped');process.exitCode=1;});
