...
in this repository. Only its test-source phase is active, under the exact six paths and
restrictions in [the Sol handoff](CODEX_SOL_BBD_WAL_003_TESTS.md). No production source,
execution, evidence, Git, package, workflow, native binary, Rust install, wallet, node,
network, hardware, or device action is authorized for the source actor.
