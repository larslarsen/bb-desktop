...
raw_path=Path('wallet-broker/target/wal015-live-policy-green-01.json')
evidence_path=Path('docs/testing/BBD-WAL-015-LIVE-POLICY-GREEN-01.md')
try:
    metadata()
    changes=json.loads(Path('docs/handoff/WAL_015_POLICY_REVIEWED_DELTAS.json').read_text())
    old=json.loads(Path('wallet-broker/target/wal015-live-policy-red-01.json').read_text())['final_hashes']
    for name,swaps in changes.items():
        text=Path(name).read_text()
        for before,after in reversed(swaps):
            assert text.count(after)==1
            text=text.replace(after,before)
        assert hashlib.sha256(text.encode()).hexdigest()==old[name], 'unrelated policy edits changed'
    record['unrelated_policy_edits_preserved']=True
    required(['node','test/walletLiveSyncPolicy.node.js'],60)
    baseline=json.loads(Path('wallet-broker/target/wal013-final-window-security-04.json').read_text())
    ...
    assert row['exit']==1 and failures==baseline['policy_current_failures'], 'policy ratchet differs'
    record['policy_current_failures']=failures
    row=command(['node','scripts/security-policy.js'],60)
    assert row['exit']==1 and 'wallet Rust source inventory is missing or extra' in row['output']
    ...
    assert build['stop']=='policy failure ratchet changed' and build['hot_journal_falsification_detected']
    assert all(pins[p]==h for p,h in build['final_hashes'].items() if p not in changes)
    ...
    record['reused_validation']='111 distinct Rust tests; 28 distinct app JS groups; native pointer UI; production Clippy; actual recovery falsification/restoration; Xvfb and actual-host window proof; Cargo audit unchanged transitive graph'
    record['accepted']=True
except Exception as exc: record['stop']=str(exc)
finally: finish('WAL-015 exact dependency authority and preserved policy baseline')
