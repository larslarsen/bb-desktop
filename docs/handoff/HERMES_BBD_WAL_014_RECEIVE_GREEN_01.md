# WAL-014 receive production validation

Reviewer accepted Sol actor29022 exit0, same documented source session. Inspected all
four production diffs: native-only guard, borrowed deadline-checked session, SecretBytes
copies, strict existing-store UFVK binding and atomic issuance, native copy/lifecycle
clearing. Independently proved both test changes since red are only the authorized
network-parent cleanup guards. Input source pins are embedded. No dependency changes.

Hermes executes ONLY exact embedded launcher once, TERMINAL with explicit repo workdir,
background=true notify_on_complete=true; process.wait <=60 seconds until actual EXIT.
No execute_code, other preflight/discovery, extra tests, source design or Git integration.
This driver authorizes formatting only six listed Rust files through stdin/stdout,
focused Rust regressions/Clippy, two exact temporary falsifications with restoration,
staged broker rebuild, existing JS and owned-profile native rendering/lifecycle tests,
unchanged six-failure policy ratchet and pinned Gitleaks directory scan. No user account
profile access or app restart. Actual display smoke uses its own fresh0700 broker root.
Cargo target verified ext4 this turn. Existing unchanged dependency audit evidence is
retained; release/installer/SBOM work is outside this slice. All actors must close.

Formatting hashes are separately recorded; final hashes must match those formatted
inputs. Reviewer will inspect final formatted source and actual native screenshots
before accepting integration. Any new failure stops; record actual diagnostics.

Exact launcher:
```text
python3 -c 'from pathlib import Path; s=Path("docs/handoff/HERMES_BBD_WAL_014_RECEIVE_GREEN_01.md").read_text(); code=s.split("```python\n# WAL014_GREEN_DRIVER\n",1)[1].split("\n```",1)[0]; exec(compile(code,"WAL014_GREEN_DRIVER","exec"))'
```
...
try:
    ...
    required(cargo+['test']+flags+['--test','account_management','--test','vault_session','--test','zec_address','--test','zec_store','--test','zec_hygiene'],300)
    required(cargo+['test']+flags+['--features','native-ui','--test','account_native_ui','--test','native_surface'],300)
    required(cargo+['clippy']+flags+['--features','native-ui','--lib','--bin','bitbook-wallet-broker','--test','account_management','--test','account_native_ui','--','-D','warnings'],300)
