# WAL-013 manifest discovery and remaining security verification

After source actor collected closed, Hermes nous/meituan/longcat-2.0:free executes only this driver. Final03 accepted native normal/open-suppression/restored green, npm0 vulnerabilities and Rust no vulnerability failures (existing atomic-polyfill maintenance warning). Six newly failing policy groups traced to redundant explicit test manifest block; Sol removes only that block, byte-identical prior HEAD. Verify actual12 account tests still auto-discovered, rebuild/stage final manifest (no source change), exact six inherited policy failures and inventory blocker, then pinned Gitleaks Git+dir. No repeat other accepted checks. No dependency/policy/installer changes or integration. All40 input pins frozen below.

TERMINAL TOOL ONLY never execute_code, exact launcher ONCE explicit repo cwd, background true, process.wait <=60 until EXIT. No preflight/extra commands or other actors. Existing ext4 target. No artifact replay.

```sh
python3 -c 'from pathlib import Path; s=Path("docs/handoff/HERMES_BBD_WAL_013_FINAL_SECURITY_04.md").read_text(); code=s.split("```python\n# WAL013_FINAL04\n",1)[1].split("\n```",1)[0]; exec(compile(code,"WAL013_FINAL04","exec"))'
```
...
import re
raw_path=Path('wallet-broker/target/wal013-final-window-security-04.json')
evidence_path=Path('docs/testing/BBD-WAL-013-FINAL-WINDOW-SECURITY-04.md')
...
try:
    ...
    out=required([str(Path.home()/'.cargo/bin/rustup'),'run','1.98.0','cargo','test','--manifest-path','wallet-broker/Cargo.toml','--locked','--offline','--no-default-features','--test','account_management'],300,env)
    assert 'test result: ok. 12 passed; 0 failed' in out, 'account test auto-discovery failed'
