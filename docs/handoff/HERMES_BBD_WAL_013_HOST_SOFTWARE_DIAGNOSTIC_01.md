# WAL-013 native host graphics compatibility probe

Diagnostic02 actor95893 exited0; raw native error is `glutin error: [a5] GLXBadFBConfig`. Hermes verbal summary misidentified test timeout as native error; ignore that summary. Runtime source and original binary restored. This establishes actual graphics context creation failure on owner's NVIDIA/XWayland setup, independent of Xvfb rendering success.

Only diagnostic execution: unchanged exported smoke on host, intercept its actual broker spawn to add fixed LIBGL_ALWAYS_SOFTWARE=1; if unsuccessful, try same plus __GLX_VENDOR_LIBRARY_NAME=mesa. No ambient GPU variables forwarded, no package/driver install, no sandbox changes, no source/test mutation. Instrumented spawn intentionally differs from harness recorded options, so even a passing run is diagnostic only, not acceptance. First working fixed option set records captures from only the new empty owned test window and stops. Each run retains all real-window pixel/hide/reopen/EOF/cleanup assertions. No user profile or existing application modifications. Source frozen and hash pinned.

Hermes nous/meituan/longcat-2.0:free exact TERMINAL launcher only, explicit workdir/background true/notify_on_complete, process.wait <=60 until EXIT. No execute_code, preflight, extra commands, source edits or Git. Existing ext4 target. Report actual working_overrides or failure; reviewer inspects raw and images.

```sh
python3 -c 'from pathlib import Path; s=Path("docs/handoff/HERMES_BBD_WAL_013_HOST_SOFTWARE_DIAGNOSTIC_01.md").read_text(); code=s.split("```python\n# WAL013_HOST_SOFTWARE01\n",1)[1].split("\n```",1)[0]; exec(compile(code,"WAL013_HOST_SOFTWARE01","exec"))'
```
