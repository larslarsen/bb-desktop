...
raw_path=Path('wallet-broker/target/wal015-live-policy-red-01.json')
evidence_path=Path('docs/testing/BBD-WAL-015-LIVE-POLICY-RED-01.md')
try:
    metadata()
    row=command(['node','test/walletLiveSyncPolicy.node.js'],60)
    failures=[s for s in row['output'].splitlines() if s.startswith('not ok ')]
    assert row['exit']==1 and len(failures)==1 and 'current manifest' in failures[0]
    assert 'wallet Rust manifest dependency pins or features differ from review' in row['output']
    record['expected_red']=True
    record['accepted']=True
except Exception as exc: record['stop']=str(exc)
finally: finish('WAL-015 exact manifest policy RED')
