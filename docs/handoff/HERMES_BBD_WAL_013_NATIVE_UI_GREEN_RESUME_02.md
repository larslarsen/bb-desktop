# WAL-013 native widget resume02 — authorized

Resume01 rejected: Hermes22552/session20260910_111829_369d91 used execute_code twice
instead of terminal, lacked HERMES_SESSION_ID, performed no formatter/tests, and then
listed outputfiles. It also wrote /tmp/wal013_launcher.py and verbally misreported
cause. Rawrecord says missing current session; prior evidence retained. No acceptance
command beyond3metadata commands executed. Sourcepins unchanged. New raw/evidence
names below prevent overwrite; no completedtest replays. MUST USE TERMINAL TOOL.
NEVER execute_code, Python sandbox, generatedtempfile or rewrittenlauncher. Readfile
then paste exact shelllauncher into terminal with explicitrepoworkdir, backgroundtrue,
notify_on_complete. Collect through process.wait only. Stopfirstunexpected, no retry.
...
```sh
python3 -c 'from pathlib import Path; s=Path("docs/handoff/HERMES_BBD_WAL_013_NATIVE_UI_GREEN_RESUME_02.md").read_text(); code=s.split("```python\n# WAL013_NATIVE_UI_GREEN_RESUME_02\n",1)[1].split("\n```",1)[0]; exec(compile(code,"WAL013_NATIVE_UI_GREEN_RESUME_02","exec"))'
```
...
import difflib,re
raw_path=Path('wallet-broker/target/wal013-native-ui-green-resume-02.json')
evidence_path=Path('docs/testing/BBD-WAL-013-NATIVE-UI-GREEN-RESUME-02.md')
