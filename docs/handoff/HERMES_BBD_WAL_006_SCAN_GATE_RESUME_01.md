# Hermes Handoff — BBD-WAL-006 Scan Gate Resume 01
...
Correction Review 01, Scan Gate Formatter Review 01, Scan Format Capture Review 01, Scan Format
...
reuse the earlier preconditions, formatter result, capture result, identity, or command result.
This resume replaces only the protected governance parent and five-source inventory below. Every
other protected identity, absent-path requirement, disk-path requirement, exact command and
order, expected count, stop rule, evidence requirement, staging restriction, commit message,
push, and final-state proof in Scan Gate 01 remains mandatory.
...
Total source is 5,262 lines. Re-prove the 12 protected non-source identities, absent
...
the exact clean final state. Never stage the ignored capture or target paths. The reviewer alone
