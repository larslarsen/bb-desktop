# WAL-013 native widget finish — authorized

Reviewer accepts Sol33845 closedexit0: two directKeypatterns remove redundantguards
withoutchangingactions; restoretest now uses actualCancel if replay opensCreateform
and assertsactualRestorebackupcontrol before continuing. Confirm_calls==0 and absent
Confirmrestore assertions preserved; formerambiguousCreateheadingcheck strengthened.
UI5f0baa71/test95584435 source reviewed. Prior8widgetgreens and10nativewidgetregression
green recorded; latterunchanged andnotrerun. Knownrestoretestcallsite1182 fromexact
backtrace provednavigationassumption. Allotheractorsclosed, source/tests frozen.

Hermes nous/poolside/laguna-s-2.1:free only: exacttwofilemechanicalformatter,9widget
regressions (keypatternchanged), restoreconfirmation bypassfails thenrestorecasegreen,
native lib/binClippy and no-default bincheck. No10widgetreplay, actualwindow/build
alreadygreen and finalfalsification/build/security follows. No extra commands/Git/
investigation/delegation. TERMINAL TOOL ONLY, neverexecute_code. Exactlauncheronce,
explicitrepo workdir backgroundtrue;process.wait<=60 untilEXIT. Stopunexpectedfirst.

```sh
python3 -c 'from pathlib import Path; s=Path("docs/handoff/HERMES_BBD_WAL_013_NATIVE_UI_GREEN_FINISH_01.md").read_text(); code=s.split("```python\n# WAL013_NATIVE_UI_GREEN_FINISH_01\n",1)[1].split("\n```",1)[0]; exec(compile(code,"WAL013_NATIVE_UI_GREEN_FINISH_01","exec"))'
```
...
import difflib,re
raw_path=Path('wallet-broker/target/wal013-native-ui-green-finish-01.json')
evidence_path=Path('docs/testing/BBD-WAL-013-NATIVE-UI-GREEN-FINISH-01.md')
...
try:
    metadata()
    format_paths=['wallet-broker/src/account_ui.rs','wallet-broker/tests/account_native_ui.rs']
    ...
    required(cargo+['check','--manifest-path','wallet-broker/Cargo.toml','--locked','--offline','--no-default-features','--bin','bitbook-wallet-broker'],180)
