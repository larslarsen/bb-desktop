# Codex Luna Handoff — BBD-WAL-004 Final Green Integration 6
...
Require `HEAD == origin/master` at the governance parent, clean index, exactly the
accepted 15 production paths plus six Rust test paths, and no extra path. Match every
SHA-256 in GREEN_3 except these accepted final values:
...
26475b2ccddd692b036e5440fdfde66d105f943f6bde912d81391efe7984b76e  wallet-broker/tests/vault_crypto.rs
```

The lockfile remains
`1d182550f82b9dc443804d82e709b5d58021d4243bef1612c440aac59df1c2f5`.
Require `vault_crypto.rs` to differ only by accepted Correction 6 and the other five test
paths to differ only by rustfmt layout. Stop on any unaccepted semantic test change.
...
vulnerabilities, all 78 Rust tests and the independent vector, Clippy/all-targets/all-
features, native compile, and RustSec green; no canary/scratch residue, unrelated change,
window, package build, wallet, node, device, or network client. Stop without staging on
any failure. Do not install cargo-deny/cyclonedx or manually trigger GitHub.

## Evidence and integration

On exact green create only `docs/testing/BBD-WAL-004-GREEN.md` with preflight, formatter
contingency/final hashes, every command/status/count/tool, the one accepted compatibility
test and five formatter-only test paths, no-canary/scratch result, npm/RustSec results,
deferred local-tool note, and all final hashes. Update only `docs/handoff/CURRENT_TASK.md`
to `PRODUCTION GREEN INTEGRATED — FALSIFICATION AND CI SECURITY/SBOM GATES PENDING` and
link evidence.
