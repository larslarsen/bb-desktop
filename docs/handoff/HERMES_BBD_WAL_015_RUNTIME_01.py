...
try:
    ...
    for name in format_paths:
        ...
        result=subprocess.run(argv,input=Path(name).read_bytes(),stdout=subprocess.PIPE,stderr=subprocess.PIPE,timeout=60)
    ...
    cargo=[str(Path.home()/'.cargo/bin/rustup'),'run','1.98.0','cargo','test','--manifest-path','wallet-broker/Cargo.toml','--locked','--offline','--no-default-features','--features','native-ui']
    for extra in [['--test','zec_live_sync'],['--test','account_management','wal015_'],['--test','account_native_ui','wal015_'],['--test','zec_live_transport'],['--test','zec_live_validation'],['--test','zec_live_recovery']]:
        row=command(cargo+extra,240)
        assert row['exit'] in (0,101) and not row.get('timeout'), 'unexpected runtime status'
        assert 'could not compile' not in row['output'], 'compiler correction required before runtime checks'
        assert 'test result:' in row['output'], 'test execution not reached'
    record['production_accepted']=False
    record['diagnostic_capture_complete']=True
    record['accepted']=True
except Exception as exc: record['stop']=str(exc)
finally: finish('WAL-015 focused runtime diagnostics and regression RED')
