# Hermes Handoff — BBD-WAL-008 Phase-C Slice-02 Green 01
...
the complete `zec_hardware` test, and the three frozen source paths.
...
`git diff --check`. If the failure or restoration is not exact, stop without another
execution command.
...
Require the focused test to pass 18/0; the affected Rust tests to pass 11/8/8 for
`zec_prepare`/`zec_store`/`zec_hygiene`; the wallet contract to pass 48; both compile
gates to exit 0 without warnings/diagnostics; Node policy to have exactly 86 `ok` and
no `not ok`; and the security script to end with
`BitBook desktop security policy checks passed.` Stop at the first mismatch. No accepted
source, test, manifest, lockfile, or policy file may mutate.
...
invoke another actor, run a product/device/network command, touch Monero/WAL-007, or
