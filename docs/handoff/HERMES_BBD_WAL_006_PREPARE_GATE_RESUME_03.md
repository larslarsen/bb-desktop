# Hermes Handoff — BBD-WAL-006 Prepare Gate Resume 03
...
Accepted source identities are exact in Prepare Format Correction Review 03. Frozen tests remain
the two identities in Resume 01. Read completely: `AGENTS.md`, `TESTING.md`, the original prepare
gate handoff, all three gate-format reviews and accepted format-correction reviews, this handoff,
the four changed sources, both frozen tests, and `CURRENT_TASK.md`.
...
`git diff --check`, and ext4 location for both WAL-006 Cargo work directories. Stop on any mismatch
or extra path.
...
state/actor/active-handoff block of `docs/handoff/CURRENT_TASK.md`. Record all three format stops,
their accepted corrections, and every evidence item required by the original gate.

Stage exactly the four accepted sources, the evidence file, and `CURRENT_TASK.md`. Commit exactly
