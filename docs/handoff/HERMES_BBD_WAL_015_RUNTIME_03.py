...
try:
    ...
    for target in ['zec_live_transport','zec_live_validation','zec_live_recovery']:
        row=command(cargo+['--test',target],240)
        assert row['exit']==0 and 'test result:' in row['output'], 'focused test failed'
    record['accepted']=True
except Exception as exc: record['stop']=str(exc)
finally: finish('WAL-015 remaining focused runtime checks')
