...
raw_path=Path('wallet-broker/target/wal015-live-green-02.json')
evidence_path=Path('docs/testing/BBD-WAL-015-LIVE-GREEN-02.md')
try:
    metadata()
    record['formatted_files']=[]
    for name in ['wallet-broker/src/accounts.rs','wallet-broker/src/zec/test_support.rs']:
    ...
    for extra in [['--test','zec_live_sync'],['--test','account_management','wal015_']]:
        required(cargo+extra,240)
...
finally: finish('WAL-015 final lint, recovery falsification, native rebuild and security ratchets')
