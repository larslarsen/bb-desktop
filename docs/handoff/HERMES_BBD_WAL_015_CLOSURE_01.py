...
raw_path=Path('wallet-broker/target/wal015-governance-closure-01.json')
evidence_path=Path('wallet-broker/target/wal015-governance-closure-01.md')
try:
    assert len(sys.argv)==2 and len(sys.argv[1])==64
    assert hashlib.sha256(Path(__file__).read_bytes()).hexdigest()==sys.argv[1]
    pins['docs/handoff/HERMES_BBD_WAL_015_CLOSURE_01.py']=sys.argv[1]
    metadata()
