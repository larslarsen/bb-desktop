# WAL-013 restore diagnostic and pending native checks — authorized

Hermes24468/session20260910_112009_3f8342 correctly usedterminal;8of9widgets passed,
including actualcreate/unlock/lock/restartpersistence. One restoretest missinglabel
remains; correctedfieldhelperfixedpreviousinput failures. Sourceunchanged, testformatter
only recorded; allpinsmatch. Reviewer suspects stale confirm-coordinate replay hits
another valid listaction, causingtestscene mismatch. Need exactbacktracecallsite.
Hermes only nous/poolside/laguna-s-2.1:free; exact knownfailingcase+RUST_BACKTRACE=1
EXPECTED101, then previouslyunrun10nativewidgets andClippy. No full9groupreplay.
No edits/formatters/falsifications/Git/preflight/otheractors. TERMINAL TOOL ONLY;
NEVER execute_code. Exactlauncheronce, explicitrepo workdir backgroundtrue,wait<=60
untilEXIT. StopfirstUNEXPECTED; knownrestorefailure isexpecteddiagnostic, notgreen.

```sh
python3 -c 'from pathlib import Path; s=Path("docs/handoff/HERMES_BBD_WAL_013_NATIVE_UI_DIAGNOSTIC_01.md").read_text(); code=s.split("```python\n# WAL013_NATIVE_UI_DIAGNOSTIC_01\n",1)[1].split("\n```",1)[0]; exec(compile(code,"WAL013_NATIVE_UI_DIAGNOSTIC_01","exec"))'
```
...
raw_path=Path('wallet-broker/target/wal013-native-ui-diagnostic-01.json')
evidence_path=Path('docs/testing/BBD-WAL-013-NATIVE-UI-DIAGNOSTIC-01.md')
...
try:
    metadata()
    env=dict(os.environ);env['RUST_BACKTRACE']='1'
    row=command(cargo+['test']+common+['--test','account_native_ui','restore_requires_distinct_confirm_and_cannot_replay','--','--exact'],120,env)
    assert row['exit']==101 and '0 passed; 1 failed;' in row['output'], 'expected known restore case failure'
    assert 'stack backtrace' in row['output'], 'missing callsite diagnostic'
    row=command(cargo+['test']+common+['--lib','native_ui::'],180)
    assert row['exit']==0 and '10 passed; 0 failed;' in row['output'], 'native regression failure'
    required(cargo+['clippy']+common+['--lib','--bin','bitbook-wallet-broker','--','-D','warnings'],240)
    record['accepted']=True
except Exception as exc:
    record['stop']=str(exc)
finally:
    finish('WAL-013 known restore widget diagnostic and pending native checks')
