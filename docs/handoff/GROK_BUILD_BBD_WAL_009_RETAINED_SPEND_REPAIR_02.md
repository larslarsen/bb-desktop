# WAL-009 retained-spend repair 02
...
Read AGENTS.md, TESTING.md, this handoff, only the leading active section of
docs/handoff/CURRENT_TASK.md, and
docs/testing/BBD-WAL-009-RETAINED-SPEND-API-REVIEW.md.
...
## Public API route resolving repair 01's stop

The wire type pczt::orchard::Spend has NO value getter. Do not call one.
Use the public pczt::roles::verifier::Verifier role. Its with_ironwood closure
receives &orchard::pczt::Bundle. This different parsed type has public
Bundle::actions, Action::spend, Spend::value -> &Option<NoteValue>, and
NoteValue::inner -> u64. Verifier is always exported; with_ironwood requires
only the already enabled orchard feature. No manifest or dependency edit is needed.

Use ownership through this role: Verifier::new takes the existing Pczt by value,
with_ironwood performs the bounded checks and extracts only the required scalar
values, and finish returns the Pczt for the next existing pipeline stage. The
closure returns Result<(), pczt::roles::verifier::OrchardError<E>>; choose an
explicit concrete E and use Custom for application validation failures. Never
mistake successful parsing for signature/proof verification.

The private capture helper may consume Pczt and return (Pczt, RetainedSpendBinding).
First run existing validate_prepared on the ORIGINAL retained representation;
then capture its unique unsigned index/key/nullifier, inspect both parsed spend
values through the role, require selected Some(positive) and other Some(0), and
return the role's Pczt with the immutable binding. Carry that returned owner into
the real Signer. Do not re-run the original MemoPlaintext-only prepared check on
the role's reserialized representation: parsing may expand compact output fields.

Similarly inspect_final_pczt may consume the finalized Pczt and return
(Pczt, ClearEffects). Preserve its existing output/memo/fee/proof checks on the
incoming representation; check retained index/key/nullifier and both signatures,
then use the role to require the selected value equals the retained positive value
and the other is Some(0). Return that same role's owner to TransactionExtractor.
A small private shared owner-consuming helper that returns (Pczt, [u64; 2]) is
permitted instead of duplicating the value inspection. Check length == 2 before
indexing and require both values present. New value/role validation failures map
to INTENT_MISMATCH; preserve existing signature and prepared-shape error mappings.

Do not clone PCZTs, bundles, keys, or secrets to use this new inspection route;
do not change the existing synthetic signer clone in this slice. Do not add a
second wire codec, Debug/serde extraction, private-field access, unsafe code, or
value inference from outputs, fees, witnesses, dummy keys, or supplied metadata.
The positive-real/zero-padding predicates remain mandatory. This changes only
private ownership plumbing inside the four authorized paths; the accepted decoded
transaction context/verifier remains frozen. Parsing/reserialization stays inside
the pinned PCZT library and must not introduce new fixture or native work.
...
Also authorized read-only: pczt 0.9.3 roles/verifier/mod.rs,
roles/verifier/orchard.rs, roles.rs; Orchard 0.15.5 pczt.rs and
value.rs::NoteValue; backend 0.24.0 Cargo.toml's pczt feature entry.
