...
raw_path=Path('wallet-broker/target/wal015-live-secrets-diagnostic-01.json')
evidence_path=Path('docs/testing/BBD-WAL-015-LIVE-SECRETS-DIAGNOSTIC-01.md')
try:
    metadata()
    ...
    report=Path('wallet-broker/target/wal015-gitleaks-redacted-01.json')
    assert not report.exists()
    row=command([scanner,'dir','--redact=100','--no-banner','--report-format','json','--report-path',str(report),'.'],180)
    assert row['exit'] in (0,1)
    findings=json.loads(report.read_text())
    record['findings']=[{k:f.get(k) for k in ['RuleID','File','StartLine','EndLine','Fingerprint','Description']} for f in findings]
    record['redacted_report_sha256']=hashlib.sha256(report.read_bytes()).hexdigest()
    record['diagnostic_complete']=True
    record['accepted']=True
except Exception as exc: record['stop']=str(exc)
finally: finish('WAL-015 redacted secret-scan finding locations')
