...
manifest/mode/hash, diffcheck, npm/cargoaudit, exactlegacy6policyfailure+Rustinventory
ratchet, pinnedGitleaksGit+dir as final01 unrunchecks. No installers/dependencies/tools
or other sourceupdates. Networkonlyrequiredadvisorydata. Existingext4target reused.

TERMINAL TOOL ONLY neverexecute_code, exactlauncheronce explicitrepoworkdir,
backgroundtrue notifycompletion, process.wait<=60untilEXIT. No preflight/extra
commands/Gitstagecommitpush/delegation/investigation/recorddesign. Stopfirstunexpected;
finallyrestore source+productionbuild evenfaulttestfailure. Allcompletedrecordsremain.

```sh
python3 -c 'from pathlib import Path; s=Path("docs/handoff/HERMES_BBD_WAL_013_FINAL_WINDOW_SECURITY_02.md").read_text(); code=s.split("```python\n# WAL013_FINAL_WINDOW_SECURITY_02\n",1)[1].split("\n```",1)[0]; exec(compile(code,"WAL013_FINAL_WINDOW_SECURITY_02","exec"))'
```
...
import re
raw_path=Path('wallet-broker/target/wal013-final-window-security-02.json')
evidence_path=Path('docs/testing/BBD-WAL-013-FINAL-WINDOW-SECURITY-02.md')
...
try:
    ...
    row=command(cargo+['test']+common+['--test','account_native_ui'],180)
    assert row['exit']==0 and '9 passed; 0 failed;' in row['output'], 'visibility change widget regression'
    required(cargo+['clippy']+common+['--lib','--bin','bitbook-wallet-broker','--','-D','warnings'],180)
    ...
    assert row['exit']==0 and 'tests passed (1)' in row['output'], 'corrected initial window not green'
