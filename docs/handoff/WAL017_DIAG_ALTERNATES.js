...
const endpoints={zaino:'https://zaino.testnet.unsafe.zec.rocks:443',zecpro:'https://lwd.testnet.zec.pro:443',nighthawk:'https://testnet.lightwalletd.com:9067'};
if(!Object.hasOwn(endpoints,choice))throw new Error('unapproved endpoint');
const root='wallet-broker/target/wal017-diagnostic-'+choice;
const endpoint=endpoints[choice];
...
(async()=>{
  ...
  ]){const result=await rpc(method,body,label);record.commands.push(result);if(result.error||result.grpc_status!=='0')break;}
  fs.writeFileSync(path.join(root,'summary.json'),JSON.stringify(record,null,2)+'\n',{flag:'wx',mode:0o600});
  fs.writeFileSync('docs/testing/BBD-WAL-017-DIAGNOSTIC-'+choice+'.md','# WAL017 bounded public RPC diagnostic\n\n```json\n'+JSON.stringify(record,null,2)+'\n```\n',{flag:'wx'});
