# Codex Luna Handoff — BBD-WAL-004 Final Green and Integration
...
WAL-004 review and red/green evidence, `CURRENT_TASK.md`, all source, all tests, policy,
deny configuration, validator, and workflows.

## Preflight and frozen state

Require `HEAD == origin/master` at the governance parent and a clean index. The worktree
must contain exactly the accepted 15 production paths plus formatter-only modifications
to the six Rust test paths, with no extra path. Match these SHA-256 hashes:
...
6014c99d1bdef16ff2b554c2a9e778ac711f05ba82b6fe08e3f49fe6531732b5  wallet-broker/src/vault.rs
...
1c1da0425460154ea84a0751bfb9f40d0cea58b15efbf13aaebd0c463bde60b8  wallet-broker/src/native_ui.rs
...
The immutable committed lockfile remains SHA-256
`1d182550f82b9dc443804d82e709b5d58021d4243bef1612c440aac59df1c2f5`.
Inspect the full six-test diff and require every change to be canonical rustfmt layout,
with no semantic change. Stop on any changed identifier, literal, operator, call,
assertion, branch, test name, or case membership.

## Complete green gate

Use `/home/lars/.cargo/bin/rustup run 1.98.0`, locked/offline Cargo where shown, the
ignored disk-backed repository target, and no `/tmp`. Run separately in this order:
...
Do not mutate formatting: the current state must pass fmt check. Network is authorized
only for the exact npm audit and final Git push. Success requires all npm suites, all 65
direct policy cases, zero npm vulnerabilities, all 78 Rust integration tests and the
independent vector, Clippy/all-features, native-feature compile, and RustSec audit green;
no canary or scratch residue, unrelated change, native window, package build, wallet,
node, device, or network client. Stop without staging on any failure. Do not install
missing cargo-deny/cyclonedx or trigger a manual GitHub run; their pinned workflows are
post-push gates.

## Evidence and integration

On exact green create only `docs/testing/BBD-WAL-004-GREEN.md` with every command,
status, actual count, tool version, no-canary/scratch result, formatter-only test
manifest, npm/RustSec result, deferred local-tool note, and final hashes. Update only
`docs/handoff/CURRENT_TASK.md` to `PRODUCTION GREEN INTEGRATED — FALSIFICATION AND CI
SECURITY/SBOM GATES PENDING` and link the evidence.

Run `git diff --check`. Stage exactly the 15 production paths, six formatter-only Rust
tests, evidence, and `CURRENT_TASK.md`. Inspect the full staged diff and names; the six
tests must remain semantically equivalent. Commit once as
`feat: add encrypted wallet custody core` and push master. Require final
`HEAD == origin/master`, no non-ignored change, and report the commit, complete manifest,
evidence line/hash, all results/counts/tools, final hashes, and push. The reviewer owns
the separate post-commit falsification handoff and post-push GitHub gate review.
