# WAL-013 native widget green — authorized

Reviewer accepts corrected UI743095e48f6ef607137e4211ed0c91a22bde099153b7a1dc602b7294de8cbd6f
(948lines) and lib9b39a0f7. Sourceactors41323/47428/30264 collectedexit0.
Privateerror access and doublefocus fixed; secret moves via exactsize preallocatedbox,
originalfullcapacity wiped withoutreallocation; both eventcollections scrubbed without
skippinghidden deadlines/repaints. One-shotrestore/privateport/closederrors reviewed.
Runtime da1d1484 reviewed;92distinctJSgroups and bothmenu/EOFfalsifications accepted.
Hermes67481 transcript used cdprefix plus exactlauncher; no extras/source drift.
All sourceactors closed; all source/tests FROZEN except explicit formatter/falsify below.

Hermes nous/poolside/laguna-s-2.1:free only. Execute exactdriver once. Mechanical
rustfmt1.98 on ONLY account_ui.rs/runtime.rs/lib.rs with skip_children=true; recordfull
formatterdiffs andpins. Then9actualwidget tests inclrealmanagerpersistence,10existing
nativewidgetregressions, exactrestoreconfirmation suppression/retest/restoregreen,
warning-denied nativefeature lib/binClippy. No allfeatures/parkedXMR/fullZcashproof.
No source/testdesign, additionalcommands/Git/evidence/delegation. Stopfirstunexpected
result preserving completedsteps; no replay. Target remains existingdiskbackedext4,
no caches/dependencies moved/downloaded. Driver writes exactraw/evidenceonly.
Read only handoff, terminal explicitrepo workdir backgroundtrue, notifyoncomplete,
process.wait<=60 untilactualexit. Do not stop afterlaunch.

```sh
python3 -c 'from pathlib import Path; s=Path("docs/handoff/HERMES_BBD_WAL_013_NATIVE_UI_GREEN_01.md").read_text(); code=s.split("```python\n# WAL013_NATIVE_UI_GREEN_01\n",1)[1].split("\n```",1)[0]; exec(compile(code,"WAL013_NATIVE_UI_GREEN_01","exec"))'
```
...
try:
    metadata()
    format_paths=['wallet-broker/src/account_ui.rs','wallet-broker/src/runtime.rs','wallet-broker/src/lib.rs']
