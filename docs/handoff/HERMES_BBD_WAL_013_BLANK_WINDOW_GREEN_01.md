# WAL-013 XWayland correction: real rendered green and blank falsification

Sol62035/session01a08cd5-bd7b-7dc2-87a9-5e1a756b05d2 exited0. Reviewer accepts exactly two allowlist line removals plus explanatory comments, all other production unchanged. Final source pins below. Prior red execution accepted. Transcript correction to production handoff: Hermes red also ran read-only ls on its two exact evidence paths after process poll/log; no mutations or extra acceptance checks. Results/pins/actual process EXIT are valid; do not repeat red for this workflow deviation.

Hermes nous/meituan/longcat-2.0:free executes only driver below. Four focused JS suites, Xvfb rendered first/reopened windows, exact temporary AccountWindow::ui early-return mutant/build must fail blank-content assertion; finally restore native bytes/build, restored rendered green. Then same smoke on host DISPLAY=:0/XAUTHORITY using a fresh temporary empty store, no host profile/account access. This briefly opens/closes only owned test windows, captures only their client pixels, and verifies normal EOF/no forced signals. The user's existing Wayland application/window is not closed or modified. Host EGL warnings reported by owner; rendered host proof is required, no graphics workaround or package changes authorized here. Review both generated PNGs before acceptance. Final binary must remain exact original hash. Policy six inherited failures and pinned directory Gitleaks; committed scan occurs at integration. No redundant audits: dependency graph unchanged and prior audits accepted.

All source/test inputs frozen except exact temporary mutation and restoration. Existing ext4 target. No dependency installs, additional diagnostics, actor delegation, source edits, or Git. TERMINAL ONLY, never execute_code, exact launcher once explicit workdir/background true/notify_on_complete; process.wait <=60 until EXIT. Do not issue extra ls/log/preflight commands; exact driver records evidence. No output replay.

```sh
python3 -c 'from pathlib import Path; s=Path("docs/handoff/HERMES_BBD_WAL_013_BLANK_WINDOW_GREEN_01.md").read_text(); code=s.split("```python\n# WAL013_BLANK_GREEN01\n",1)[1].split("\n```",1)[0]; exec(compile(code,"WAL013_BLANK_GREEN01","exec"))'
```
...
raw_path=Path('wallet-broker/target/wal013-blank-window-green-01.json')
evidence_path=Path('docs/testing/BBD-WAL-013-BLANK-WINDOW-GREEN-01.md')
...
try:
    ...
    required(window,90)
    target=Path('wallet-broker/src/account_ui.rs');saved=target.read_bytes()
    original='if self.quit_sent || !self.control.visible() {'
    assert saved.decode().count(original)==1
    try:
        target.write_text(saved.decode().replace(original,'if true {',1))
        required(['node','scripts/build-wallet-broker.js'],300,env)
        row=command(window,90)
        assert row['exit']==1 and 'native account window remained blank' in row['output'], 'blank rendered-content falsification not detected'
        record['blank_falsification_detected']=True
    finally:
