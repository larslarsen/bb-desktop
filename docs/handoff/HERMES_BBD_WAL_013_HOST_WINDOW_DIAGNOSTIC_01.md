# WAL-013 actual host XWayland startup diagnostic

Green01 actor28358/session20260910_124136_2eefcb exited0, driver1. Four JS suites, Xvfb real rendered first/reopened, blank-fault detection, restored source/build/Xvfb green accepted. Reviewer inspected PNG: title/testnet/payment notice/Create account/Restore backup plus disabled Unlock/Lock/Export visible. Host smoke failed before manage: did not initialize hidden within5sec. This is not accepted actual-host proof. Exact native source restored. Need observations before any further production change.

Hermes nous/meituan/longcat-2.0:free only exact diagnostic driver: run unchanged exported smoke on actual DISPLAY=:0 with instrumentation of child spawn/close, broker stderr <=32KiB, and matching-title window inspect output. Only the test's own new temporary empty profile is used; never read actual user accounts, host window contents, other process environment or logs. No source/test modification or in-memory assertion rewrite. Diagnostic observation only, no acceptance credited. No extra commands, builds, Git or dependencies. All source hashes frozen. Existing ext4 target.

TERMINAL ONLY exact launcher once explicit workdir, background true/notify_on_complete, process.wait <=60 until EXIT. Never execute_code, preflight, ls or extra commands. Output evidence records exact trace and process closure.

```sh
python3 -c 'from pathlib import Path; s=Path("docs/handoff/HERMES_BBD_WAL_013_HOST_WINDOW_DIAGNOSTIC_01.md").read_text(); code=s.split("```python\n# WAL013_HOST_DIAG01\n",1)[1].split("\n```",1)[0]; exec(compile(code,"WAL013_HOST_DIAG01","exec"))'
```
...
raw_path=Path('wallet-broker/target/wal013-host-window-diagnostic-01.json')
evidence_path=Path('docs/testing/BBD-WAL-013-HOST-WINDOW-DIAGNOSTIC-01.md')
if raw_path.exists() or evidence_path.exists():raise RuntimeError('existing artifacts')
try:
    metadata()
    row=command(['node','-e',trace],90)
    assert row['exit'] in (0,1) and '"label":"close"' in row['output'], 'diagnostic did not collect child close'
    record['diagnostic_only']=True
    record['accepted']=True
except Exception as exc:record['stop']=str(exc)
finally:finish('WAL-013 actual host window diagnostic; not acceptance')
