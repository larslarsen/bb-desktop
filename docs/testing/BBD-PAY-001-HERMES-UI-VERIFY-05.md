# BBD-PAY-001 — Hermes UI verification 05
...
Command: `CHROME_DEVEL_SANDBOX=/opt/google/chrome/chrome-sandbox BBD_PAY001_ARTIFACT_DIR=dist/pay001-ui-verify05/ui-01 timeout --signal=TERM --kill-after=5s 300s xvfb-run -a dist/pay001-finish-verify01/runtime/electron test/paymentInbox.electron.js`
Exit: 1. Failed.

## Failure: pointer did not close details before focus proof

The keyboard fix from review 13 worked — the UI journey progressed past the
keyboard interaction stage to pointer testing. The new failure is on pointer
click not closing expanded payment details before the focus proof assertion.

This is a different, later failure than both the prior `keyboard did not close
details` (UI04) and `configured ready empty payment state missing` (UI03).

## Sandbox status (verified working)

```json
{
  "browser": { "pid": 746266, "Seccomp": "0", "NoNewPrivs": "0", "NSpid": "746266" },
  "renderer": { "pid": 746312, "Seccomp": "2", "NoNewPrivs": "1", "NSpid": "746312\\t4" }
}
```
...
- `connection: "HTTP only"` — connected via HTTP
- `fixture_failures: 1` — social POST /ob/chat returned 403 (expected: POST not in origin-allowed methods)
- `blocker.unexpected: 0` — no unexpected blocked requests
- Social fixture: 10 hits, 9 GET 200 + 1 POST 403
...
| fixture-request-card-980x720.png | 980x654 | `be2f05f8cb0ae21d8a0e034e4decd6fd0d088bfba3e0a3ffdadfc4591d7969eb` |
| fixture-failure.png | 980x654 | `dfa8aed91ea715f6ebaef80f346ef2870ca689b0177e8bc1f996ff5f2748f03e` |
...
- `dist/pay001-ui-verify05/actor.txt`
- `dist/pay001-ui-verify05/source-before.json`
- `dist/pay001-ui-verify05/driver-source.js`
- `dist/pay001-ui-verify05/preflight.json`
- `dist/pay001-ui-verify05/runtime-before.json`
- `dist/pay001-ui-verify05/syntax.stdout.log`
- `dist/pay001-ui-verify05/syntax.stderr.log`
- `dist/pay001-ui-verify05/ui.stdout.log`
- `dist/pay001-ui-verify05/ui.stderr.log`
- `dist/pay001-ui-verify05/ui-01/bootstrap-diagnostics.json`
- `dist/pay001-ui-verify05/ui-01/sandbox-status.json`
- `dist/pay001-ui-verify05/ui-01/ui-failure-diagnostics.json`
- `dist/pay001-ui-verify05/ui-01/fixture-empty-messages-1440x1000.png`
- `dist/pay001-ui-verify05/ui-01/fixture-request-card-980x720.png`
- `dist/pay001-ui-verify05/ui-01/fixture-failure.png`

## Verdict

UI RUN FAILED — `pointer did not close details before focus proof`. The keyboard
fix worked: journey progressed past keyboard interaction to pointer testing. The
new failure is on pointer click not closing expanded payment details.
